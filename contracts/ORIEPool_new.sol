// SPDX-License-Identifier: MIT  0.6.10
...
// File: bscContracts/ORIEPool.sol
...
contract ORIEPool is Ownable {
    ...
    modifier onlySetter() {
        require(isSetter(msg.sender), "ORIEPool: Not the setter");
        _;
    }
    ...
    function getSetter(uint256 _index) public view returns (address){
        require(_index <= getSetterLength() - 1, "ORIEPool: index out of bounds");
    ...
    function withdraw(uint256 _pid, uint256 _amount, uint256 _lockedId) public {
        ...
        uint256 penlaty = 0;
        if (_amount > 0) {
            require(!user.lockedInfo[_lockedId].isWithdrawed, "ORIEPool: This amount of lockedId is withdrawed");
            require(user.lockedInfo[_lockedId].amount == _amount, "ORIEPool: Invalid amount of lockedId");
    ...
    function addSetter(address _newSetter) public onlyOwner returns (bool) {
        require(_newSetter != address(0), "ORIEPool: NewSetter is the zero address");
        return EnumerableSet.add(_setter, _newSetter);
    }

    function delSetter(address _delSetter) public onlyOwner returns (bool) {
        require(_delSetter != address(0), "ORIEPool: DelSetter is the zero address");
    ...
    function addRewardToken(
        ...
        require(_startTime > block.timestamp, "ORIEPool: invalid start time");
        require(!isExistedRewardToken[_rewardToken], "ORIEPool: existed reward token");
        massUpdatePools();
    ...
    function depoistRewardToken(uint256 amount, uint256 rid) onlySetter external {
        require(setterOfRid[rid] == msg.sender, "ORIEPool: incorrect setter of this reward token pool");
        massUpdatePools();
        ...
        require((currBal - prevBal) == amount, "ORIEPool: incorrect balance after depositing");

// SPDX-License-Identifier: MIT 0.6.12
...
contract OriswapPools is Ownable {
    using SafeMath for uint256;
    ...
    modifier notPause() {
        require(paused == false, "OriswapPools: Mining has been suspended");
        _;
    }
    ...
    constructor(
        ...
        require(_startTime > block.timestamp, "OriswapPools: Incorrect start time");
        DOG = _dog;
    ...
    function deposit(uint256 _pid, uint256 _amount) public payable notPause {
        ...
        if (pool.poolType == POOL_TYPE.Single && depositSinglePoolFeeOn) {
            require(msg.value == depositSinglePoolFee, "OriswapPools: Can't deposit to single pool without fee");
    ...
    function withdraw(uint256 _pid, uint256 _amount) public payable notPause {
        ...
        if (pool.poolType == POOL_TYPE.Single && withdrawSinglePoolFeeOn) {
            require(msg.value == withdrawSinglePoolFee, "OriswapPools: Can't withdraw from single pool without fee");
            feeAddr.transfer(address(this).balance);
        }
        address _user = msg.sender;
        UserInfo storage user = userInfo[_pid][_user];
        require(user.amount >= _amount, "OriswapPools: Insuffcient amount to withdraw");
        updatePool(_pid);
        uint256 pendingAmount = user.amount.mul(pool.accDOGPerShare).div(1e12).sub(user.rewardDebt);
        if (pendingAmount > 0) {
            _safeDOGTransfer(_user, pendingAmount);
    ...
    function batchAddPools(
        ...
        require(
            ...
            "OriswapPools: Invalid length of pools"
        );
    ...
    function setPoolShare(uint256 single, uint256 lp) public onlyOwner {
        require(single.add(lp) == 100, "OriswapPools: the sum of two share should be 100");
        SINGLE_SHARE = single;
        LP_SHARE = lp;
    }
